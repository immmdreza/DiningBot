...
from src.inline_keyboards_handlers.food_priorities_handler import FoodPrioritiesHandler
...
def default_reply_markup(buttons: List[List[str]]):
    """Reply markup with a default style."""
...
class DiningBot:
    def __init__(
        ...
        log_level="INFO",
        db: DB = None,
        admin_sso_username: str = None,
        admin_sso_password: str = None,
    ):
        ...
        self.reserve_handler = ReserveMenuHandler(
            self.db, admin_sso_username, admin_sso_password
        )
    ...
    def start(self, update, context):
        if self.db.add_bot_user(
            ...
            context.bot.send_message(
                chat_id=update.effective_chat.id, text=messages.start_message
            )
    ...
    def set(self, update, context):
        ...
        if len(args) != 2:
            context.bot.send_message(
                chat_id=update.effective_chat.id, text=messages.set_wrong_args_message
            )
            return
    ...
    def inline_keyboard_handler(self, update, context):
        type = FoodPrioritiesHandler.separate_callback_data(update.callback_query.data)[
            0
        ]
        if type == "FOOD":
            _, action, choosed, page = FoodPrioritiesHandler.separate_callback_data(
                update.callback_query.data
            )
            return self.reserve_handler.inline_food_choosing_handler(
                update, context, action, choosed, int(page)
            )
        elif type == "FOODCOURT":
            _, action, choosed = FoodCourtSelectingHandler.separate_callback_data(
                update.callback_query.data
            )
            return self.reserve_handler.inline_food_court_choosing_handler(
                update, context, action, choosed
            )
        elif type == "FORGETCODE":
            _, forget_code = ForgetCodeMenuHandler.separate_callback_data(
                update.callback_query.data
            )
            self.forget_code_handler.inline_return_forget_code_handler(
                update, context, int(forget_code)
            )
        elif type == "AUTOMATIC_RESERVE":
            _, action = AutomaticReserveAlreadyActivatedHandler.separate_callback_data(
                update.callback_query.data
            )
            self.reserve_handler.inline_already_activated_handler(
    ...
    def send_main_menu(self, update, context):
        ...
        update.message.reply_text(
            ...
            reply_markup=default_reply_markup(MAIN_MENU_CHOICES),
        )
        return MAIN_MENU_CHOOSING
    ...
    def send_to_all(self, update, context):
        ...
        threading.Thread(
            target=self.send_message_to_all_handler,
            args=(
                context,
                msg,
            ),
    ...
    def update_user_favorite_foods(self, update, context):
        update.message.reply_text(text=messages.update_foods_started_message)
        # /update_foods <week>
        splited_text = update.message.text.split()
        week = 1
        if len(splited_text) == 2:
    ...
    def setup_handlers(self):
        ...
        update_food_list_handler = CommandHandler(
            "update_foods", self.update_user_favorite_foods
        )
        ...
        menue_handler = ConversationHandler(
            ...
            states={
                ...
                INPUT_USERNAME: [
                    MessageHandler(
                        ...
                        & ~(
                            Filters.command
                            | Filters.regex(INPUT_USERNAME_AND_PASSWORD_EXCLUDE)
                        ),
                ...
                INPUT_FOOD_NAME: [
                    MessageHandler(
                        Filters.text
                        & ~(Filters.command | Filters.regex(INPUT_FORGET_CODE_EXCLUDE)),
            ...
            fallbacks=[
                MessageHandler(
                    Filters.regex(BACK_TO_MAIN_MENU_REGEX), self.send_main_menu
                )
            ],
        )
